...
function sortTable(event, elem) {
    var key = elem.key, field = elem.field, direction = elem.direction;
...
function table(elem) {
    ...
    var _k = localState.field, sortField = _k === void 0 ? undefined : _k, _l = localState.direction, sortDirection = _l === void 0 ? undefined : _l;
    ...
    while (ix < rowsLen) {
        ...
        for (var _m = 0; _m < groups.length; _m++) {
            var grouped = groups[_m];
            var collected = [];
    ...
    if (editRow) {
        ...
        for (var _q = 0; _q < fields.length; _q++) {
            var field = fields[_q];
