        ...
        while (ix < len && subItem.eav.attribute === item.eav.attribute) {
            ...
            if (!subItem.isManual) {
                ...
                if (state.active && state.active.__id === subItem.eav.__id) {
                    ...
                    valueUI.t = "input";
                    valueUI.value = "= " + app_1.eve.findOne("query to id", { id: subItem.sourceView }).query;
...
function submitAttribute(event, elem) {
    console.log("SUBMIT!");
    var eav = elem.eav, sourceView = elem.sourceView;
    var chain;
    if (elem.sourceView !== undefined) {
        //remove the previous source
        chain = app_1.dispatch("remove attribute generating query", { eav: eav, view: sourceView });
    }
    else {
        //remove the previous eav
        chain = app_1.dispatch("remove sourced eav", { eav: eav });
    }
    handleAttributeDefinition(eav.entity, eav.attribute, event.currentTarget.value, chain);
}
//---------------------------------------------------------
// Wiki Widgets
//---------------------------------------------------------
